...
def map_columns(df):
    ...
    for col in df.columns:
        ...
        elif col_lower in ["credit", "kredit"]:  # detect target column automatically
            df_renamed.rename(columns={col: "credit"}, inplace=True)
    return df_renamed
...
if uploaded_file is not None:
    ...
    # Check if credit column exists
    if "credit" not in user_data.columns:
        st.error("⚠️ Uploaded dataset must include the target column (credit).")
        st.stop()

    # Drop rows with missing target
    user_data = user_data.dropna(subset=["credit"])

    # Append safely to main dataset
